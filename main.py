...
def main():
    ...
    try:
        ...
        lstm = load_model(f'model/lstm/{args.location}.h5', 
                         custom_objects={"mse": tf.keras.losses.MeanSquaredError()})
        gru = load_model(f'model/gru/{args.location}.h5',
                        custom_objects={"mse": tf.keras.losses.MeanSquaredError()})
        saes = load_model(f'model/saes/{args.location}.h5',
                         custom_objects={"mse": tf.keras.losses.MeanSquaredError()})
        xgboost = xgb.Booster()
        xgboost.load_model(f'model/xgboost/{args.location}.json')
        
        # Load location mapping
        location_to_idx = np.load('model/location_mapping.npy', allow_pickle=True).item()
        
        models = [lstm, gru, xgboost, saes]
        names = ['LSTM', 'GRU', 'XGBoost', 'SAEs']
    except Exception as e:
