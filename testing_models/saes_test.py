...
# Load SAE Model
model = tf.keras.models.load_model('../model/saes.h5', 
...
X_test = np.reshape(X_test, (X_test.shape[0], X_test.shape[1]))  # SAE doesn't need 3D input
...
plt.title("Actual vs Predicted Lane Flow (SAE Model)")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()